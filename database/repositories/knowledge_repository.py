...
def make_chunk_id(source, content_hash):
    return f"KNOW_{uuid5(NAMESPACE_URL, f'{source}:{content_hash}').hex[:12].upper()}"


def upsert_knowledge_chunk(source, title, content, content_hash, embedding):
    chunk_id = make_chunk_id(source, content_hash)
...
def delete_stale_knowledge_chunks(active_chunk_ids):
    ...
    with get_connection() as connection:
        cursor = connection.execute(query, tuple(active_chunk_ids))
        connection.commit()
